...
def configuration():
    ...
    - val_size = 1 - (test_size + train_size)  # Tamaño porcentual del conjunto de validación
    - runs = 10
    ...
    - val_auc = False  # Si es True, la validación será guiada en función de AUC, si es false en función de G-Mean
    - imbalanced_degree = None  Variable que se utilizará para encontrar las clases minoritarias en tiempo de ejecución
    - pintar_graficas = Si es True, sacará una gráfica en cada ejecución
    - rmv_dup = False
    - rmv_noise = False
    """
    test_size = 0.5  # Tamaño porcentual del conjunto de test
    ...
    n = 0.7
    val_auc = False  # Si es True, la valiadación será guiada en funcion de AUC, si es false en funcion de G-Mean
    imbalanced_degree = None  # Variable que se utilizará para encontrar las clases minoritarias en tiempo de ejecución
    pintar_graficas = False
    rmv_dup = True
    rmv_noise = True

    return test_size, train_size, val_size, runs, beta, k, n, val_auc, imbalanced_degree, pintar_graficas, rmv_dup, \
           rmv_noise
...
def particion_datos(inputs, outputs, test_size=0.5, train_size=0.5, val_size=0):
    """ Realiza la partición de los datos. Crea un conjunto de entrenamiento y otro de test, y si se desea puede crear
        ...
        Recibe los siguientes parámetros:
            ...
            - val_size (opcional): Porción estratificada de la base de datos que se utilizará en validación (default 0)
        Devuelve:
            ...
            - test_outputs: Matriz con las variables de salida de
              test.
            - val_inputs(opcional): Matriz con las variables de entrada de
              validación.
            - val_outputs(opcional): Matriz con las variables de salida de
              validación.
    """
    ...
    if not (0 <= test_size <= 1 and 0 <= train_size <= 1 and 0 <= val_size <= 1):
        sys.exit("En particion_datos: Los tamaños relativos deben estar todos entre 0.0 y 1.0")

    train_inputs = train_outputs = test_inputs = test_outputs = val_inputs = val_outputs = None
...
def generate_synthetics_ig(inputs, outputs, index, minority_label, normalizedrlist, k, n, G, val_inputs, val_outputs,
                           test_inputs, test_outputs, val_auc=False, return_clf=True, graphics=False):
    """ Genera patrones sintéticos utilizando una metaheurística y los incluye en el conjunto de datos.
        Recibe los siguientes parámetros:
            ...
            - val_outputs: Matriz con las variables de salida del conjunto de validación.
            - val_auc: Si es True, la validación será guiada en función de AUC, si es false en función de G-Mean
              (default False)
            - return_clf (opcional): Si es True, devuelve el árbol entrenado durante la validación (default False)
        Devuelve:
    ...
    neighborhood_outputs = np.append(outputs, synthetics_outputs[: G - destroyed], axis=0)
    contador = 0
    mejor_validacion = -1
    mejor_clf = None
    val_array = 0
    test_array = 0
    first_newinputs_looparray = 0
    better_newinputs_looparray = 0
    original_newinputs_looparray = 0
    ...
    while contador < 100 and normalizedrlist.sum():
        ...
        if mejor_validacion == -1:
            first_newinputs = np.copy(newdata_inputs)

        validacion, clf = validation(newdata_inputs, newdata_outputs, val_inputs, val_outputs, val_auc=val_auc,
                                     return_clf=True)
        ...
        if validacion > mejor_validacion:
            ...
            contador += 1
            validacion = mejor_validacion

        testeo = validation(mejor_newinputs, newdata_outputs, test_inputs, test_outputs, clf=mejor_clf,
                            val_auc=val_auc)
        ...
        normalizedrlist, k = ratio_borde(minority_inputs, neighborhood_inputs, neighborhood_outputs,
                                         minority_label=True)

    # se crea el vector de resultados con arboles aleatorios
    for x in range(150):
        first_test = validation(first_newinputs, newdata_outputs, test_inputs, test_outputs, val_auc=val_auc)
        first_newinputs_looparray = np.hstack((first_newinputs_looparray, first_test))

        mejor_test = validation(mejor_newinputs, newdata_outputs, test_inputs, test_outputs, val_auc=val_auc)
        better_newinputs_looparray = np.hstack((better_newinputs_looparray, mejor_test))

        original_test = validation(inputs, outputs, test_inputs, test_outputs, val_auc=val_auc)
        original_newinputs_looparray = np.hstack((original_newinputs_looparray, original_test))

    if graphics:
        paint(val_array[1:], test_array[1:])
    all_loop_dates(val_array, test_array, first_newinputs_looparray, better_newinputs_looparray,
                   original_newinputs_looparray)
...
def better_choice(distancias, indices, normalizedrlist, g, n=0.5):
    ...
    # elegidos = np.random.choice(indices, g, replace=True)

    return elegidos


def validation(train_inputs, train_outputs, test_inputs, test_outputs, clf=None, val_auc=False, return_clf=False):
    """ Calcula el g_mean o el AUC sobre el conjunto de validación para guiar a la metaheuristica
        Recibe los siguientes parámetros:
            ...
            - clf (opcional): Árbol ya entrenado (Si no se proporciona se creará y entrenará un árbol en la ejecución
            de la función)
            - val_auc: Si es True, la validación será guiada en función de AUC, si es false en función de G-Mean
            - return_clf: Si es True, también devuelve el árbol entrenado (default False)
            - return_auc: Si es True, devuelve AUC en lugar de G_mean como precisión de la validación (default False)
        Devuelve:
            - g_mean: Variable de precisión sobre el conjunto de validación (default)
            - auc: Área bajo la curva ROC. Variable de precisión sobre el conjunto de validación (Opcional)
        """
    ...
    if val_auc:
        ...
        score = np.mean(np.sqrt((float(tp) / float(tp + fn)) * (float(tn) / float(tn + fp))))
...
def paint(val_array, test_array, to_screen=True, to_csv=False, file_name='grafica'):
    """ Muestra una gráfica con los valores de test y validación durante el transcurso de la metaheurística. De esta
    forma es posible observar si hay sobre-entrenamiento
        Recibe los siguientes parámetros:
        - val_array: Vector con los mejores resultados de g-mean conseguidos en el conjunto de validación
        - test_array: Valores de test conseguidos utilizando el mismo árbol y el mismo conjunto de entrenamiento que
         en validación, pero en esta ocasión utilizando el conjunto de test
        - to_screen (opcional): Si es True, imprime las medidas por pantalla (default True)
        - to_csv (opcional): Si es True, creara un fichero con las medidas calculadas (default True)
        - file_name (opcional): Nombre del fichero que será creado con las medidas. Solo tendrá sentido si toCsv=True.
        """
    ...
    if to_screen:
        ...
        pj = plt.plot(t, val_array, 'g-', t, test_array, 'r-', first_value, 'b--')
...
def all_loop_dates(val_array, test_array, first_array, better_array, original_array):
    name_val = 'validaciones.csv'
    name_test = 'testeos.csv'
    name_first = 'firstSet_Trees.csv'
    name_better = 'betterSet_Trees.csv'
    name_original = 'originalSet_Trees.csv'
    try:
        ...
        file_original = pd.read_csv(('./' + name_original), header=None)
        exist = 1
    except IOError:
        exist = 0
    ...
    better_array = better_array.reshape(-1, 1)
    original_array = original_array.reshape(-1, 1)
    ...
    if exist:
        ...
        original = np.array(file_original.values[:, :], dtype='float')

        diference = val.shape[0] - val_array.shape[0]
        if diference > 0:
            val_array = np.append(val_array[:, 0], np.ones(diference) * val_array[-1][0], axis=0)
            test_array = np.append(test_array[:, 0], np.ones(diference) * test_array[-1][0], axis=0)
        elif diference < 0:
            val_store = np.append(val[:, 0], np.ones(-1 * diference) * val[-1][0], axis=0)
            test_store = np.append(test[:, 0], np.ones(-1 * diference) * test[-1][0], axis=0)
            for x in range(1, val.shape[1], 1):
                val_store = np.c_[val_store, np.append(val[:, x], np.ones(-1 * diference) * val[-1][x], axis=0)]
                test_store = np.c_[test_store, np.append(test[:, x], np.ones(-1 * diference) * test[-1][x], axis=0)]

            val = val_store
            test = test_store

        val_store = np.c_[val, val_array]
        test_store = np.c_[test, test_array]
        first_store = np.c_[first, first_array]
        better_store = np.c_[better, better_array]
        original_store = np.c_[original, original_array]
    else:
        val_store = val_array
        test_store = test_array
        first_store = first_array
        better_store = better_array
        original_store = original_array

    df = pd.DataFrame(val_store)
    df2 = pd.DataFrame(test_store)
    df3 = pd.DataFrame(first_store)
    df4 = pd.DataFrame(better_store)
    df5 = pd.DataFrame(original_store)
    df.to_csv(name_val, header=False, index=False, sep=',')
    df2.to_csv(name_test, header=False, index=False, sep=',')
    df3.to_csv(name_first, header=False, index=False, sep=',')
    df4.to_csv(name_better, header=False, index=False, sep=',')
    df5.to_csv(name_original, header=False, index=False, sep=',')
...
def borar_archivos():
    ...
    if os.path.exists(name_first):
        os.remove(name_first)

    if os.path.exists(name_better):
        os.remove(name_better)

    if os.path.exists(name_original):
        os.remove(name_original)


def principal(fichero, semillas, test_size, train_size, val_size, beta=1, k_original=5, n=0.5, val_auc=False,
              imbalanced_degree=None, graphics=False, rmv_dup=False, rmv_noise=False):
    """ Función principal que se encarga de ejecutar el sistema
        Recibe los siguientes parámetros:
            - fichero: Ruta de la base de datos incluyendo el nombre del archivo.
            - semillas: Vector de semillas para controlar la aleatoriedad del algoritmo
            - test_size: Porción estratificada de la base de datos que se utilizará en el test (default is 0.5)
            - train_size: Porción estratificada de la base de datos que se utilizará en el entrenamiento (default  0.5)
            - val_size (opcional): Porción estratificada de la base de datos que se utilizará en validación (default 0)
            - beta: Especifica que el nivel de balanceo que se conseguirá después de la generación de sintéticos
            (β = 1 significa totalmente balanceado)
            - k_original: Valor de k originalmente introducido por el usuario
            - n: Factor de ponderación entre cercanía al borde o cercanía al primer padre
            - val_auc: Si es True, la validación será guiada en función de AUC, si es false en función de G-Mean
            - imbalanced_degree (opcional): Grado de desbalanceo para determinar las clases minoritarias. Si no se
            - rmv_dup: Si es True, se activará la función para eliminar los duplicados
            - rmv_noise: Si es True, se activará la función para eliminar el ruido
    ...
    # Estructura de datos donde se almacenará las medidas de precisión
    datos = np.empty((9, 0))
    ...
    for loop in xrange(0, runs, 1):
        ...
        train_inputs, train_outputs, test_inputs, test_outputs, val_inputs, val_outputs = particion_datos(inputs,
                                                                                                          outputs,
                                                                                                          test_size,
                                                                                                          train_size,
                                                                                                          val_size)

        minority_index = class_index(train_outputs, True)  # índices de la clase minoritaria
        ...
        if rmv_noise and (clases[0][1] / float(clases[1][1])) > 0.1:  # Se elimina ruido
            normalizedrlist, k, rlist = ratio_borde(minority_inputs, train_inputs, train_outputs, True, k,
                                                    return_rlist=True)  # ratio de cercanía normalizado
        ...
        newdata_inputs, newdata_outputs, mejor_clf = generate_synthetics_ig(train_inputs, train_outputs, minority_index,
                                                                            minority_label, normalizedrlist, k, n, G,
                                                                            val_inputs, val_outputs, test_inputs,
                                                                            test_outputs, val_auc, return_clf=True,
                                                                            graphics=graphics)
        ...
        datos = tester(newdata_inputs, newdata_outputs, test_inputs, test_outputs, datos, clf=mejor_clf)

    imprimir_resultado(datos.mean(axis=1), to_screen=True, to_csv=False)
...
if __name__ == "__main__":
    ...
    borar_archivos()
    test_size, train_size, val_size, runs, beta, k, n, val_auc, imbalanced_degree, graphics, rmv_dup, rmv_noise = \
        configuration()
    ...
    if not (isinstance(val_auc, (int, bool))):
        print "val_auc vale", val_auc, "pero deberia tener un valor igual a True o False"
    ...
    if val_size == 0:
        print "ANEIGSYN necesita un conjunto de validación para funcionar, vuelve a configurar el algoritmo " \
              "para dar un tamaño a este conjunto. Por ejemplo 0.2"
        sys.exit("Valor erroneo para val_size")

    np.random.seed(semilla)
    semillas = np.random.choice(np.asarray(list(map("".join, permutations('12345678'))), dtype=int), runs)

    principal(fichero, semillas, test_size, train_size, val_size, beta, k, n, val_auc=val_auc,
              imbalanced_degree=imbalanced_degree, graphics=graphics, rmv_dup=rmv_dup, rmv_noise=rmv_noise)
